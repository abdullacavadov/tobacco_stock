...
<body class="sb-nav-fixed">
                    ...
                    <div class="card mb-4">
                        <div class="card-header d-flex justify-content-between">
                            <span>
                                ...
                                Xammal satışın reallaşdırılması
                            </span>
                                        ...
                                        <div class="form-floating">
                                            <select class="form-control" id="name" name="name">
                                                ...
                                                <?php foreach ($rows as $row): ?>
                                                    ...
                                                    if ($row['type'] == 'raw') {
                                                        $row['type'] = 'Xammal';
                                                    } elseif ($row['type'] == 'flavour') {
                                                        $row['type'] = 'Aroma';
                                                    } elseif ($row['type'] == 'package') {
                                                        $row['type'] = 'Qab';
                                                    } elseif ($row['type'] == 'label') {
                                                        $row['type'] = 'Etiket';
                                                    } elseif ($row['type'] == 'cover') {
                                                        $row['type'] = 'Paket';
                                                    }
                                                    ?>
                                                        ...
                                                        <option value="<?= htmlspecialchars($row['id']) ?>">
                                                            <?= htmlspecialchars(
                                                                $row['type'] . ' - ' . $row['name'] . ' (' . $row['supplier'] . '), 1' . $row['unit'] . ' mayası - ' . number_format($row['price'] / $row['stock'], 3) .
                                                                "₼ (stokda: " .
                                                                number_format($row['stock'], 2) . ' ' . $row['unit'] .
                                                                ", part: " . date('d.m.Y', strtotime($row['in_stock'])) . ")"
                                                            ) ?>
                                                        </option>
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <div class="form-floating">
                                            <input class="form-control" id="qty" name="qty" type="number" step="0.001"
                                                placeholder="100" />
                                            <label for="qty">Miqdar (kq/əd)</label>
                                        </div>
                                    </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            const form = document.getElementById("create_raw_order");
            ...
            form.addEventListener("submit", function (e) {
                ...
                fetch("./ajax/create_raw_order.php", {
                    method: "POST",
                    body: formData
                })
